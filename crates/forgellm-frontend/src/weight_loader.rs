...
///
/// Input loads are amortised across the 4 rows: each 32-byte Q8_0 sub-block
/// is read once and dotted against 4 weight rows' qs slices.  The
/// `Σ x_q8` reduction is also computed once per sub-block (independent of
/// the weight row).
#[cfg(target_arch = "aarch64")]
pub fn dot4_q4_k_q8_0_neon(
    w0: &[u8],
    w1: &[u8],
    w2: &[u8],
    w3: &[u8],
    ...
    const Q4K_BYTES: usize = 144;
    const Q8_BYTES: usize = 34;
    assert!(k.is_multiple_of(Q4K_BLOCK), "Q4_K dot4 requires k multiple of 256");
    ...
    for sb in 0..num_sb {
        ...
        for r in 0..4 {
            ...
            d_arr[r] = f16_to_f32(d_bits);
            dmin_arr[r] = f16_to_f32(dmin_bits);
        }
        let x_block_base = sb * 8 * Q8_BYTES;
        ...
        for chunk in 0..4 {
            ...
            // Per-row (sub_scale, sub_min) for both sub-blocks in this chunk.
            ...
            unsafe {
                ...
                // Load input for sub-block 1 (low nibbles).
                let x1_off = x_block_base + is * Q8_BYTES;
                let x1_scale = f16_to_f32(u16::from_le_bytes([
                    input_q8[x1_off],
                    input_q8[x1_off + 1],
                ...
                // Load input for sub-block 2 (high nibbles).
                let x2_off = x_block_base + (is + 1) * Q8_BYTES;
                let x2_scale = f16_to_f32(u16::from_le_bytes([
                    input_q8[x2_off],
                    input_q8[x2_off + 1],
...
mod tests {
    ...
    #[cfg(target_arch = "aarch64")]
    #[test]
    fn dot4_q4_k_q8_0_matches_four_dot1_calls() {
        // Build 4 distinct super-blocks; verify dot4 == [dot1, dot1, dot1, dot1].
        let configs = [
            (0.03f32, 0.01f32, [3u8, 5, 7, 2, 10, 4, 6, 8], [1u8, 2, 3, 1, 2, 1, 1, 2]),
            (0.05, 0.02, [4, 8, 12, 16, 20, 24, 28, 32], [1, 2, 3, 4, 5, 6, 7, 8]),
            (0.07, 0.03, [10, 11, 12, 13, 14, 15, 16, 17], [2, 3, 4, 5, 6, 7, 8, 9]),
            (0.02, 0.005, [1, 2, 3, 4, 5, 6, 7, 8], [0, 1, 0, 1, 0, 1, 0, 1]),
        ];
        let qs_lo: [[u8; 32]; 4] = std::array::from_fn(|c| {
            std::array::from_fn(|i| ((c * 4 + i / 8) % 16) as u8)
        });
        ...
        let weights: Vec<Vec<u8>> = configs
            .iter()
            .map(|(d, dmin, sc, m)| build_q4k_superblock(*d, *dmin, *sc, *m, &qs_lo, &qs_hi))
            .collect();

        let input_f32: Vec<f32> = (0..256)
            .map(|i| (i as f32 * 0.07).sin() * 0.4 - 0.05)
            .collect();
        let input_q8 = quantize_f32_to_q8_0(&input_f32);

        let scalar = [
            dot_q4_k_q8_0(&weights[0], &input_q8, 256),
            dot_q4_k_q8_0(&weights[1], &input_q8, 256),
            dot_q4_k_q8_0(&weights[2], &input_q8, 256),
            dot_q4_k_q8_0(&weights[3], &input_q8, 256),
        ];
        let (n0, n1, n2, n3) =
            dot4_q4_k_q8_0_neon(&weights[0], &weights[1], &weights[2], &weights[3], &input_q8, 256);
        let neon = [n0, n1, n2, n3];

        for r in 0..4 {
            let abs = (scalar[r] - neon[r]).abs();
            let rel = abs / scalar[r].abs().max(1e-6);
            assert!(
                rel < 1e-4,
                "row {r}: scalar={} dot4={} rel={rel}",
                scalar[r],
                neon[r]
            );
        }
